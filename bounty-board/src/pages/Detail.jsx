    ...
    <div className="p-4 pb-20 min-h-screen">
      ...
      <div className="bg-paper p-6 rounded-lg border-4 border-wood shadow-xl max-w-md mx-auto relative">
        ...
        {role === 'admin' && (
            <div className="absolute top-4 right-4 flex gap-2 z-10">
                <button 
                    ...
                    className="p-2 bg-blue-700 text-white rounded shadow hover:bg-blue-800 border border-wood/50"
                    title="Edit Data"
                >
                    <Edit size={18}/>
                </button>
                <button 
                    ...
                    className="p-2 bg-red-800 text-white rounded shadow hover:bg-red-900 border border-wood/50"
        ...
        <div className="space-y-3 text-wood font-serif">
          <div className="flex justify-between border-b border-wood/20 pb-1">
             <span className="font-bold opacity-70">ALIAS</span>
             <span className="font-bold">{bounty.alias || '-'}</span>
          </div>
          <div className="flex justify-between border-b border-wood/20 pb-1">
             <span className="font-bold opacity-70">CRIME</span>
             <span className="text-red-700 font-black uppercase">{bounty.crime}</span>
          </div>
          <div className="flex justify-between border-b border-wood/20 pb-1">
             <span className="font-bold opacity-70">REWARD</span>
             <span className="font-black text-xl">$ {parseInt(bounty.bounty_amount).toLocaleString()}</span>
          </div>
          <div className="flex justify-between border-b border-wood/20 pb-1">
             <span className="font-bold opacity-70">LAST SEEN</span>
             <span>{bounty.last_seen}</span>
          </div>
          
          <div className="pt-2">
             <p className="text-sm opacity-70 font-bold mb-1">DESCRIPTION:</p>
             <p className="bg-stone-200/50 p-2 rounded border border-wood/10 italic text-sm">{bounty.description}</p>
          </div>
        </div>
        ...
        {bounty.status === 'captured' && role === 'admin' && (
           <button 
            ...
            className="w-full mt-8 bg-wood text-paper font-bold py-4 rounded border-2 border-black shadow-lg active:translate-y-1 uppercase tracking-widest hover:bg-[#4a332a]"
          >
            REVOKE CAPTURE
          </button>
        )}
